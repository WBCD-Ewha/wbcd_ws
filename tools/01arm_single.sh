...
source ~/.bashrc
workspace=$(pwd)
# /home/arx/桌面/sdk/R5Pro_SDK_KDL/ROS/R5Pro_ws/tools/01arm_single.sh
# CAN
gnome-terminal -t "can1" -x bash -c "cd ${workspace}/../../..;cd ARX_CAN/arx_can; ./arx_can1.sh; exec bash;"
sleep 1s
# R5
gnome-terminal -t "R5Pro" -x  bash -c "cd ${workspace}/..; source devel/setup.bash && roslaunch arx_r5pro_controller open_single_arm.launch; exec bash;"
